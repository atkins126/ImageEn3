...
uses neurquant, imageenview, ieview, tifccitt, jpegfilt, pngfilt, shellapi;
...
begin
  ...
  inc(rowbuf); // ESC
  rowbuf^ := 0; // EOL
  result := int64(DWORD(rowbuf)) - int64(DWORD(basbuf)) + 1;
end;
      ...
      24:
        begin // 24 bit per pixel
          for y := Bitmap.Height - 1 downto 0 do
            fs.read(pbyte(Bitmap.Scanline[y])^, lw);
          if (fs.Size-fs.Position >= lw1 * Bitmap.Height) then  // 16 Jul 2009, 9:53
          begin
            fs.Read(andmap^, lw1 * Bitmap.Height);
            amap:=andmap;
            for y:=Bitmap.Height-1 downto 0 do
...
begin
  ...
  begin
    ...
    getmem(buf, l);
    try
      if ie.IEBitmap.HasAlphaChannel (*and (integer(bc[i]) <> 24)*) then  // 16 Jul 2009, 9:53
      begin
        for row := ie.IEBitmap.Height - 1 downto 0 do
        begin
          fillchar(buf^, l, 0);
...
begin
  ...
  begin
    ...
    getmem(buf, l);
    if Bitmap.HasAlphaChannel (*and (integer(bc[i]) <> 24)*) then // 16 Jul 2009, 9:53
    begin
      for row := Bitmap.Height - 1 downto 0 do
      begin
        fillchar(buf^, l, 0);
...
begin
  ...
  else if (magic = 'P5') and (vmax<=255) then // 3.0.4
  begin
    // PGM (8bit) - RAWBITS
    IOParams.BitsPerSample := 8;
    IOParams.SamplesPerPixel := 1;
  ...
  else if (magic = 'P5') and (vmax<=65535) then // 3.0.4
  begin
    // PGM (16bit) - RAWBITS
