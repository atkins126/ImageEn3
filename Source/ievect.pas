...
const
  ...
  IEVVER: byte = 82; // current version
...
<FM>Methods and Properties<FN>
  ...
  <A TImageEnVect.ObjEnableFastDrawing>
  ...
  <A TImageEnVect.MaxMovingDistance>
  ...
  <A TImageEnVect.GetIndexFromObj>
  ...
  <A TImageEnVect.ObjBlendOperation>
...
<FM>Events<FN>
  ...
  <A TImageEnVect.OnPresentMeasure>
...
!!}
  ...
  private
    ...
    procedure DrawRuler(wcanvas: TIECanvas; QuoteLength:double; x1, y1, x2, y2: integer; um: TIEUnits; Color: TColor; PW: integer; RulerType: TIEVRulerType; zx,zy: double; var plim: trect; logfont: PLogFontA);
    procedure DrawAngle(wcanvas: TIECanvas; AnglePoints: array of TPoint; Color: TColor; PW: integer; var plim: trect; usezoom: boolean; mul: integer; zx,zy: double; logfont: PLogFontA);
    // Objects
...
<FM>Description<FN>
Pen used when user insterts a line, box or ellipse object.
!!}
...
When false (default) iekBOX objects are selectable only clicking on the border, unless <A TImageEnVect.ObjBrushStyle> is <FC>bsSolid<FN>.
When true iekBOX objects are always selectable inside the bounding box (even with ObjBrushStyle=bsClear).
!!}
...
Using ieemManual you have to interrupt manually the inserting call <A TImageEnVect.CancelInteracts> or setting a new value in <A TImageEnVect.MouseInteractVt>.
!!}
...
<FM>Declaration<FC>
ieMeasureUnits: array [<A TIEUnits>] of string = ('pixels', 'inches', 'km', 'mt', 'cm', 'mm', 'microns', 'nanometers', 'feet', 'yards', 'miles');

<FM>Description<FN>
Specifies how measure units are displayed as strings.

<FM>Example<FN>
ieMeasureUnits[ieuNANOMETERS] := 'uM';
!!}
  ieMeasureUnits: array [TIEUnits] of string = ('pixels', 'inches', 'km', 'mt', 'cm', 'mm', 'microns', 'nanometers', 'feet', 'yards', 'miles');
...
begin
  ...
  begin
    ...
    BitmapIdx := -1;
    SetObjBitmapICO(-1, OIC_HAND, IEDEFWIDTH, IEDEFHEIGHT);
    LogFont := nil;
    TextFormatRef := nil;
    TextFormat := nil;
    TextEditable := true;
...
var
  ...
  begin
    with pobj^ do
      if LogFont = nil then
      begin
        getmem(LogFont, sizeof(TLogFontA));
        copymemory(LogFont, fNewObj.LogFont, sizeof(TLogFontA));
      end;
  end;
  //
begin
...
<FC>hobj = -2<FN> represents last inserted object.
!!}
...
<FC>hobj<FN> is a value that identifies the object. hobj can be -1 (next object to insert) or -2 (last object inserted).
...
begin
  ...
  begin
    if LogFont = nil then
      getmem(LogFont, sizeof(TLogFontA));
    GetObject(v.Handle, sizeof(TLogFontA), @xlog);
    copymemory(LogFont, @xlog, sizeof(TLogFontA));
    PenColor := v.Color;
  end;
  if hobj <> -1 then
    Update;
...
begin
  ...
  begin
    ...
    Text := IEStrDup(PAnsiChar(Adjust1310(v)));
  end;
  if hobj <> -1 then
    Update;
end;
  ...
  begin
    ...
    begin
      if fShareBitmaps then
        q := FindBitmap(v) // look if the bitmap already exists
      else
...
// Looks for the object near x,y (at 100% coordinates)
...
begin
  ...
  if (result=0) and fObjBitmapHandleTransparencyOnSel and bmp.HasAlphaChannel then
  begin
...
begin
  with Obj^ do
    case Kind of
      ...
      iekLINE, iekLINELABEL:
        begin
          ...
          if Kind=iekLINELABEL then
            result := dmin( result, IEDist2Box(x,y, obj.DrawedLabelBox.Left,obj.DrawedLabelBox.Top,obj.DrawedLabelBox.Right,obj.DrawedLabelBox.Bottom,true,1) );
        end;
      iekRULER:
        begin
          result := _DistPoint2Seg(x, y, x1, y1, x2, y2);
      ...
      iekELLIPSE:
        begin
          result := IEDistPoint2Ellipse(x, y, x1, y1, x2, y2, BrushStyle<>bsClear, PenWidth);
        end;
      iekARC:
        begin
...
var
  ...
  begin
    with pobj^ do
      ...
      begin
        getmem(LogFont, sizeof(TLogFontA));
        copymemory(LogFont, aObj.LogFont, sizeof(TLogFontA));
      end;
  end;
  //
begin
  ...
  begin
    ...
    begin
      ...
      if aObj.Text <> nil then
        Text := IEStrDup(aObj.Text)
      else
        Text := IEStrDup(''); // set empty string
      AllocLogFont;
...
begin
  obj := GetObj(hobj);
  with obj^ do
    if (PolyPointsCount = 0) or (X <> PPointarray(PolyPoints)[PolyPointsCount - 1].Y) or (Y <> PPointarray(PolyPoints)[PolyPointsCount - 1].Y) then
    begin
      inc(PolyPointsCount);
  ...
  begin
    ...
    begin
      ...
      if vv = 0 then
        vv := fLastHintValue;
      fLastHintValue := vv;
      ss := IEFloatToStrFS(vv, ffFixed, fFloatPrecision, fFloatDigits) + ' ' + ieMeasureUnits[fMUnit] + '²';
    ...
    begin
      ...
      begin
        if InGrip(xx1, yy1, x, y) then
          ...
          gr := iegrBottomRight
        else if (not fUseCentralGrip) and (CalcDistPtObj(aobj,XScr2Bmp(x),YScr2Bmp(y)) < fMaxMovingDistance) then
          gr := iegrTranslation
        else if fUseCentralGrip and InGrip(oxx1 + ((oxx2 - oxx1 + 1) div 2), oyy1 + ((oyy2 - oyy1 + 1) div 2), x, y) then
          gr := iegrTranslation;
        ...
        if Kind=iekLINELABEL then
        begin
          if IEDist2Box(x,y, DrawedLabelBox.Left,DrawedLabelBox.Top,DrawedLabelBox.Right,DrawedLabelBox.Bottom,true,1) = 0 then
            gr := iegrBottomRight;
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // draw on BBitmap
        if antimul > 1 then
          fCacheBitmap.RenderToTBitmap(BBitmap, tab1, tab2, tab3, nil, x1 div antimul, y1 div antimul, w div antimul, h div antimul, x1, y1, w, h, true, false, 255, rfTriangle, false, Obj^.BlendOperation)
        else
          fCacheBitmap.RenderToTBitmap(BBitmap, tab1, tab2, tab3, nil, x1, y1, w, h, x1, y1, w, h, true, false, 255, rfnone, false, Obj^.BlendOperation);
...
!!}
procedure TImageEnVect.SaveToFileIEV(const FileName: string);
var
  fs: TFileStream;
begin
  if FileName='' then
...
!!}
function TImageEnVect.LoadFromFileIEV(const FileName: string): boolean;
var
  fs: TFileStream;
begin
  result:=false;
...
begin
  with GetObj(hobj)^ do
    if LogFont <> nil then
      strpcopy(LogFont^.lfFaceName, IECopy(v, 1, LF_FACESIZE - 1)); // strpcopy make sure that other charachetrs are ignored
  if hobj <> -1 then
    Update;
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Stream.Write(LogFont^, sizeof(TLogFontA));
          end
          else
          begin
            b := false;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        getmem(LogFont, sizeof(TLogFontA));
        Stream.Read(LogFont^, sizeof(TLogFontA));
      end;
      if ver >= 63 then
      begin
...
begin
  DrawObjectsToBitmapEx(IEBitmap, Antialias, -3);
  Update;
  ImageChange;
end;
...
begin
  DrawObjectsToBitmapEx(target, Antialias, -3);
end;
...
<FM>Description<FN>
Returns object index from object handle. This is the inverse of <A TImageEnVect.GetObjFromIndex>.
!!}
// return hobj index
...
begin
  ...
  move(pbyte(int64(DWORD(fObj)) + sizeof(integer) * (pos))^,
    pbyte(int64(DWORD(fObj)) + sizeof(integer) * (pos + 1))^,
...
procedure TImageEnVect.DrawAngle(wcanvas: TIECanvas; AnglePoints: array of TPoint; Color: TColor; PW: integer; var plim: trect; UseZoom: boolean; mul: integer; zx,zy: double; logfont: PLogFontA);
var
...
// zx= 100/ZoomX zy=100/ZoomY
// pw = pen width
procedure TImageEnVect.DrawRuler(wcanvas: TIECanvas; QuoteLength:double; x1, y1, x2, y2: integer; um: TIEUnits; Color: TColor; PW: integer; RulerType: TIEVRulerType; zx,zy: double; var plim: trect; logfont: PLogFontA);
var
      ...
      iertQUOTEBEGIN, iertQUOTECENTER, iertQUOTEEND:
        begin
          ...
          if fObjRulerQuoteHorizon and (aa > (pi/2)) and (aa <= (3*pi/2)) then
            aa := aa-pi;
          GetObject(Font.Handle, sizeof(TLogFontA), @xlog);
          xlog.lfEscapement := trunc(((-aa) * 180 / pi) * 10);
          hfont := CreateFontIndirectA(xlog);
          ...
          begin
            ...
            cx := QuoteLength;
          ss := IEFloatToStrFS(cx, ffFixed, fFloatPrecision, fFloatDigits) + ' ' + ieMeasureUnits[um];
          if assigned(fOnPresentMeasure) then
...
begin
  fObjDrawed := 0;
  DrawObjects(fIEBitmap, false, BBitmap, fObjAntialias, -3);
...
begin
  ...
  DestCanvas := TIECanvas.Create(BBitmap.Canvas, fObjAntialias and ((not fFastDrawing) or (not fObjEnableFastDrawing)), true);  // 3.0.3 (to allow antialias when softshadow=true)
...
<FM>Description<FN>
This method returns <FC>true<FN> if clipboard contains data valid for TImageEnVect.
...
<A TImageEnVect.ObjPasteFromClipboard>
!!}
function TImageEnVect.ObjIsClipboardAvailable:boolean;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      hfont := CreateFontIndirectA(aobj.LogFont^);
      lfHeight := ph;
    end;
    ...
    dx := WCanvas.TextWidth(string(aobj.Text));
    dy := WCanvas.TextHeight(string(aobj.Text));
    if lx1 < lx2 then
      x1 := lx1 - dx
    else
      x1 := lx1;
    ...
    WCanvas.TextRect(rect(x1, y1, x2, y2), x1 + ox, y1 + oy, string(aobj.Text));
...
procedure getfontrotsize(Canvas:TIECanvas; angle:double; c:AnsiChar; var x,y,w,h:integer);
var
...
procedure drawc(canvas: TIECanvas; rect: TRect; x0, y0, x1, y1: double; ch: AnsiChar; rot: integer; var plim: trect);
var
  esc, x, y, w, h: integer;
  angle: double;
  lf: TLOGFONTA;
  ...
  begin
    ...
    begin
      ...
      begin
        if Text <> nil then
          freemem(Text);
        Text := IEStrDup(PAnsiChar(AnsiString(fTextEdit.Text)));
...
You cannot use this method to load standard image files (jpeg, tiff...).
!!}
...
begin
  ...
  begin
    ...
    ph:=lfHeight;
    lfHeight := Height;
    hfont := CreateFontIndirectA(fObject.LogFont^);
    lfHeight:=ph;
  end;
