...
begin
  ...
  if length(s)>254 then
    s := AnsiString(DefTEMPPATH)+'imageentwain03.bmp';
  if length(s)>254 then
    s := 'imageentwain03.bmp';
  StrCopy(os,PAnsiChar(s));
end;
...
begin
  ...
  if not assigned(grec) then
    LogWrite('TIEProxyWin.WndProc. Msg=' + IEIntToStr(msg.message) + ' grec=' + IEIntToStr(integer(grec)) + ' grec no assigned!')
  else
    LogWrite('TIEProxyWin.WndProc. Msg=' + IEIntToStr(msg.message) + ' grec=' + IEIntToStr(integer(grec)) + ' state=' + IEIntToStr(grec^.nstate));
