...
begin
  ...
  begin
    ...
    getmem(ws, 4096);
    try
      StringToWideChar(string(fFileInput), ws, 2048);
  ...
  begin
    ...
    getmem(ws, 4096);
    try
      StringToWideChar(string(fFileOutput), ws, 2048);
      if extOut = '.avi' then
        fBuilder.SetOutputFileName(MEDIASUBTYPE_Avi, ws, multiplex, pSink)
      else if extOut = '.asf' then
...
begin
  ...
  getmem(wszPath, 4096);
  try
    StringToWideChar(string(filename), wszPath, 2048);
...
begin
  ...
  begin
    ...
    getmem(ws, 4096);
    try
      StringToWideChar(string(fFileName), ws, 2048);
