...
begin
  dec(CurPos);
  if int64(DWORD(CurPos)) < int64(DWORD(fText)) then
    CurPos := fText;
end;
...
var
  ...
  begin
    ...
    cc := c;
    while (int64(DWORD(cc)) > int64(DWORD(fetchpos))) and (cc^ <> #32) do
    begin
      o := cc;
      GoBack(cc);
      if cc = o then
    ...
    if int64(DWORD(cc)) <= int64(DWORD(fetchpos)) then
      cc := c;
    if (cc^ = #32) then
    ...
    fetch := false;
    c := fetchpos; // backtrack
    posx := CalcJust(int64(DWORD(fStopAt)) - int64(DWORD(fText)));
    rposx := posx;
    prevend := 0;
  end;
  ...
  begin
    ...
    if fcache_w[idx] = 0 then
      fcache_w[idx] := DestCanvas.TextWidth(string(cc));
  ...
  procedure PaintChar(x, y:integer; c:char);
  begin
    DestCanvas.TextRectEx(PaintableRect, x, y, string(c));
  end;
...
begin
  ...
  paintableRect := Rect(DestX+ofx, DestY+ofy, trunc(DestX+ofx+NonZoomDestWidth*fZoom), trunc(DestY+ofy+NonZoomDestHeight*fZoom));
  ...
  begin
    ...
    lastde := 0;
    repeat
      idx := int64(DWORD(c)) - int64(DWORD(fText));
  ...
  begin
    ...
    begin
      ...
      //
      PaintChar(DestX + x, DestY + y, Char(diff^.c));
      inc(diff);
    end;
    freemem(diffbuf);
