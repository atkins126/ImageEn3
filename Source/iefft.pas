...
begin
  nodes := (PIEinteger(int64(DWORD(powers)) + (nn - level) * 4))^;
  sinindx := (PIEinteger(int64(DWORD(bittabpt)) + (chunk div nodes) * 4))^;
  cosindx := (sinindx + numpts div 4) mod numpts;
  wp.real := (PIEdouble(int64(DWORD(sintabpt)) + cosindx * 8))^;
  wp.imag := direction * (PIEdouble(int64(DWORD(sintabpt)) + sinindx * 8))^;
