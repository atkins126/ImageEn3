...
begin
  ...
  begin
    ...
    begin
      fillchar(pbyte(int64(DWORD(dstbuf)) + ibi)^, (rl - q) shr 3, 0);
      inc(posw, abi);
      inc(q, abi);
    end
    else
...
///////////////////////////////////////////////////////////////////////////////////////////
// trova il primo pixel diverso da CL (0=black <>0=white)
...
begin
  ...
  begin
    ...
    begin
      ...
      if CL = 0 then
      begin
        while (ipos+4 < Width) and (pdword(db)^ = $FFFFFFFF) do
        begin
          inc(ipos, 32);
          inc(db, 4);
        end;
        while (ipos+2 < Width) and (pword(db)^ = $FFFF) do
        begin
          inc(ipos, 16);
          inc(db, 2);
        end;
        while (ipos < Width) and (db^ = $FF) do
        begin
          inc(ipos, 8);
          inc(db);
        end;
      ...
      begin
        while (ipos+4 < Width) and (pdword(db)^ = 0) do
        begin
          inc(ipos, 32);
          inc(db, 4);
        end;
        while (ipos+2 < Width) and (pword(db)^ = 0) do
        begin
          inc(ipos, 16);
          inc(db, 2);
        end;
        while (ipos < Width) and (db^ = 0) do
        begin
          inc(ipos, 8);
          inc(db);
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // set dest
      bp := pbyte(int64(DWORD(wbuf)) + (int64(wposb) shr 3));
      if r <> 0 then
...
begin
  ...
  begin
    ...
    bwr := pbyte(int64(DWORD(wbuf)) + rt)^; // bits da scrivere alla prossima
  end
  else
    bwrl := 0;
