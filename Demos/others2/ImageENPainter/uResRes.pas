...
uses
  ...
  HYIEDefs, ExtCtrls,
  IEView, Buttons, rsStorage, rsPropSaver;
...
procedure TfResize.EnableAlphChannel1Click(Sender: TObject);
begin
  ImageEnView1.EnableAlphaChannel := EnableAlphChannel1.Checked;
end;

procedure TfResize.Background1Change(Sender: TObject);
begin
...
procedure TfResize.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfResize.Edit2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if ( Key = VK_RETURN ) and ( SpeedButtonLockPreview.Down )then
    Preview;
...
procedure TfResize.RadioGroup1Click(Sender: TObject);
begin
...
begin
  fResize.ImageEnView1.Assign ( TImageENView ( FrmMain.AdvOfficePager.ActivePage.Controls
    [ 0 ] ).Bitmap );
  Edit1.Text := IntToStr ( OrgWidth );
  Edit2.Text := IntToStr ( OrgHeight );
  DontChange := false;
...
procedure TfResize.SpeedButtonLockPreviewClick(Sender: TObject);
begin
...
procedure TfResize.LockPreview1Click(Sender: TObject);
begin
